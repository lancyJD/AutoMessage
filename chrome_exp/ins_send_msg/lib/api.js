...
(function (global) {
  ...
  // 可用 chrome.storage.local 的 'sdm_api_base' 覆盖（例如后端跑在另一台机器时填 http://192.168.x.x:8787），
  // 覆盖后记得同步 manifest.json 的 host_permissions。结果缓存，避免每次请求都读 storage。
  var API_BASE_OVERRIDE_KEY = 'sdm_api_base';
  ...
  function sdmRequest(method, path, opts) {
    opts = opts || {};
    return getApiBase().then(function (base) {
      return global.SDMDevice.buildDeviceHeaders().then(function (devHeaders) {
        ...
        return fetch(url, init).then(function (res) {
          ...
          // 网络层错误（断网 / 服务未启动 / 域名不可达 / 跨机 127.0.0.1）：抛出，由调用方决定离线策略
  ...
  // 处理来自 popup 的 SDM_API 消息：{ method, path, body, token }
  // 返回可直接 sendResponse 的对象 { ok, status, body } 或 { error }
  function handleApiMessage(msg) {
    return sdmRequest(msg.method || 'GET', msg.path, { body: msg.body, token: msg.token })
      .then(function (r) { return r; })
      .catch(function (e) {
        if (e && e.network) {
          // 把浏览器真实失败原因带回 popup，便于判断是「服务没起」还是「地址不通 / 跨机」
          console.error('[SDM] API 请求失败:', (msg.method || 'GET'), msg.path, '->', e.detail, e.cause || '');
