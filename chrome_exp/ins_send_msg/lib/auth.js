...
//   - 通过 chrome.runtime.sendMessage({type:'SDM_API'}) 把请求转给 SW，自己不直连后端
//   - 提供 requireMembership() 作为插件核心功能的唯一拦截点
...
(function (global) {
  ...
  function sdmApi(method, path, opts) {
    opts = opts || {};
    return new Promise(function (resolve) {
      try {
        chrome.runtime.sendMessage(
          { type: 'SDM_API', method: method, path: path, body: opts.body, token: opts.token || null },
          function (r) {
  ...
  async function authed(method, path, body) {
    ...
    var r = await sdmApi(method, path, { token: state.accessToken, body: body });
    ...
    if (!r.network && (code === 'TOKEN_EXPIRED' || code === 'INVALID_TOKEN')) {
      ...
      r = await sdmApi(method, path, { token: state.accessToken, body: body });
    }
  ...
  async function syncMe(state) {
    var r = await sdmApi('GET', '/api/me', { token: state.accessToken });
  ...
  async function login(email, password) {
    var r = await sdmApi('POST', '/api/auth/login', { body: { email: email, password: password } });
  ...
  async function register(email, password) {
    var r = await sdmApi('POST', '/api/auth/register', { body: { email: email, password: password } });
    if (r.network) return { ok: false, error: { code: 'NETWORK_ERROR', message: '无法连接服务器，请确认后端已启动', detail: r.body && r.body.error && r.body.error.detail } };
    if (!r.body || !r.body.success) return { ok: false, error: (r.body && r.body.error) || { code: 'UNKNOWN' } };
    var d = r.body.data;
    var state = {
  ...
  async function logout() {
    ...
    if (state && state.accessToken) {
      await sdmApi('POST', '/api/auth/logout', { token: state.accessToken });
    }
    await clearState();
    return { ok: true };
  }
  ...
  async function tryRefresh(state) {
    ...
    var r = await sdmApi('POST', '/api/auth/refresh', { token: null, body: { refreshToken: state.refreshToken } });
  ...
  async function loadPlans() {
    var r = await sdmApi('GET', '/api/plans', {});
  ...
  async function getWalletInfo() {
    var r = await sdmApi('GET', '/api/payment/wallet-info');
    if (r.network) return { ok: false, error: { code: 'NETWORK_ERROR', message: '无法连接服务器' } };
    if (!r.body || !r.body.success) return { ok: false, error: (r.body && r.body.error) || { code: 'UNKNOWN' } };
  ...
  async function getSupportInfo() {
    var r = await sdmApi('GET', '/api/support-info');
    if (r.network) return { ok: false, error: { code: 'NETWORK_ERROR', message: '无法连接服务器' } };
    if (!r.body || !r.body.success) return { ok: false, error: (r.body && r.body.error) || { code: 'UNKNOWN' } };
    return { ok: true, data: r.body.data };
  }
  ...
  async function cancelSubscription() {
    var r = await authed('POST', '/api/subscription/cancel');
    if (r.network) return { ok: false, error: { code: 'NETWORK_ERROR', message: '无法连接服务器' } };
    if (r.ok) { var state = await getState(); if (state) await syncMe(state); return { ok: true }; }
    return { ok: false, error: r.error };
  }
